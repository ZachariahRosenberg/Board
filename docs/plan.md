...
- **M7** Audit view, restore UI, docs complete (api / anchors / feedback-grammar / security / deployment incl. Docker), hardening pass (caps, sanitize, headers audit), full smoke: two simulated agents + human comments → async feedback consumed via cursor and webhook — **hardening pass done 2026-09-16**: all body reads enforce their caps DURING the stream (chunked/no-content-length bodies can no longer buffer past the cap — one shared `readCappedBody` under the JSON/MCP/asset/import readers); host CSP + nosniff + no-cache now stamped on `/api` + `/mcp` responses too (were statics/assets only); sessions expire 30 days after exchange (D19, enforced at auth time, migration v6 backfills); dead `sse` subscriber kind removed from the domain (D19 — presence stays cursor/webhook-only per D13)
